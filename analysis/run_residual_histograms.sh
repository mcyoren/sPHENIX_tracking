...
usage() {
  ...
  echo
  echo "Example:"
  echo "  $0 /path/to/files '*.root' output residual_histograms.root residuals"
  echo
  echo "Important:"
  echo "  Put the input file pattern in quotes so the shell does not expand it early."
}
...
TREE_NAME="${5:-residuals}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_MACRO="${SCRIPT_DIR}/MakeResidualHistograms.C"

if [[ ! -f "${ROOT_MACRO}" ]]; then
  echo "ERROR: ROOT macro not found:"
  echo "  ${ROOT_MACRO}"
  echo
  echo "Place MakeResidualHistograms.C in the same directory as this script."
  exit 2
fi
...
echo "ROOT macro      : ${ROOT_MACRO}"
echo

# Escape characters that would otherwise break the ROOT C++ string arguments.
escape_root_string() {
  local value="$1"
  value="${value//\\/\\\\}"
  value="${value//\"/\\\"}"
...
echo
echo "Finished."
echo "Output: ${OUTPUT_DIRECTORY}/${OUTPUT_ROOT_FILENAME}"
