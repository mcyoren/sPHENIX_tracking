// ExtractK0sMomentumScaleSamples_CommonScale.C
//
// Extract per-daughter momentum-scale measurements from clean
// K0S -> pi+ pi- candidates.
//
// A single common scale is applied to both daughters:
//   p1' = s_pair p1, p2' = s_pair p2.
// The PDG K0S mass fixes s_pair. DIRA is retained only as QA.
//
...
//
// Example:
// root -l -b -q 'ExtractK0sMomentumScaleSamples_CommonScale.C(
...
namespace
{
      ...
      const double diraXYTolerance)
  {
    ...
    // The flight vector and DIRA tolerance are intentionally not used in
    // the scale extraction.  They are retained in the function interface
    // so the output and calling code remain compatible with the original
    // extractor and DIRA can still be monitored as QA.
    (void) flight;
    (void) diraXYTolerance;

    if (!finite(momentum1) || !finite(momentum2))
    {
      return result;
    }
    ...
    auto massResidual = [&](const double commonScale)
    {
      ...
      const Vec3 corrected2 =
          multiply(momentum2, commonScale);

      return invariantMass(corrected1, corrected2) -
             kKshortMass;
    };

    const double lowValue = massResidual(minimumScale);
    const double highValue = massResidual(maximumScale);

    if (!std::isfinite(lowValue) ||
        !std::isfinite(highValue) ||
    ...
    double valueLow = lowValue;
    double commonScale = 1.0;

    for (int iteration = 0; iteration < 100; ++iteration)
    {
      commonScale = 0.5 * (low + high);
      const double middleValue = massResidual(commonScale);
    ...
    result.scale2 = commonScale;
    result.scaleRatio = 1.0;
    result.massAfter =
...
void ExtractK0sMomentumScaleSamples_CommonScale(
    const char* inputDirectory = ".",
    const char* filePattern = "*.root",
    const char* outputName =
      "k0s_common_momentum_scale_samples_cut03.root",
    ...
    const Long64_t maximumEntries = -1)
{
  ...
  TH1D hCommonScale(
    "h_common_momentum_scale",
    "Common momentum scale applied to both daughters;s_{pair};candidates",
    240, minimumMomentumScale, maximumMomentumScale);

  TH2D hCommonScaleVsMassBefore(
    "h_common_scale_vs_mass_before",
    "Common scale versus original K^{0}_{S} mass;"
    "m_{#pi#pi}^{before} [GeV/c^{2}];s_{pair}",
    160, massMinimum, massMaximum,
    160, minimumMomentumScale, maximumMomentumScale);
       ...
       ++entry)
  {
    ...
    hScale2VsScale1.Fill(
      solution.scale1,
      solution.scale2);
    hCommonScale.Fill(solution.scale1);
    hCommonScaleVsMassBefore.Fill(
      massBefore,
      solution.scale1);
  ...
  TNamed methodMetadata(
    "method",
    "Apply the same momentum scale to both daughters and solve it "
    "only from the PDG KShort mass. DIRA is stored as QA and does "
    "not enter the scale extraction.");
