...
namespace
{
  ...
  struct DaughterHistSet
  {
    ...
    TH1D* h_dca_xy = nullptr;
    TH1D* h_dca_z = nullptr;
                                         ...
                                         const std::string& sampleLabel)
  {
    ...
    h.h_secondary_dca_xy_vs_phi_vs_pt = new TH3D(
        ...
        "Signed transverse PCA offset to K^{0}_{S} vertex vs #phi and p_{T}" + suffix +
            ";atan2(px,py) [rad];signed d_{xy}^{secondary} [cm];p_{T} [GeV/c]",
        96, -3.2, 3.2, 160, -2.0, 2.0, 60, 0.0, 6.0);

    h.h_secondary_dca_z_vs_eta_vs_pt = new TH3D(
        "h_secondary_dca_z_vs_eta_vs_pt",
        "PCA z offset to K^{0}_{S} vertex vs #eta and p_{T}" + suffix +
            ";#eta;d_{z}^{secondary} [cm];p_{T} [GeV/c]",
        80, -2.0, 2.0, 160, -2.0, 2.0, 60, 0.0, 6.0);
                              ...
                              const double maxAbsResidualCm)
  {
    ...
    if (std::isfinite(dcaXY)) h.h_dca_xy->Fill(dcaXY);
    if (std::isfinite(dcaZ)) h.h_dca_z->Fill(dcaZ);
...
void MakeK0sPionResidualKalmanQA_v3(
    const char* inputDir = ".",
