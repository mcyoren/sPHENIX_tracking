  ...
  const char* treeName = "residuals")
{
  ...
  for (Long64_t entry = 0; entry < nEntries; ++entry)
  {
    ...
    // Baseline track-quality selection. The |rDCA_zero| cut is applied
    // specifically to residual plots below, not to the rDCA_zero distributions.
    if (std::abs(pca_z) > 10. || ntpc_clusters < 30)
    {
      continue;
    }
