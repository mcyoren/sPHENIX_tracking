...
PHGarfield::~PHGarfield()
{
  ...
  delete m_component;
  delete m_gas;
  delete m_exCorrection3D;
  delete m_eyCorrection3D;
  delete m_ezCorrection3D;
...
{
  // Garfield uses cm, V/cm and Tesla. The ROOT field maps use cm/radians
  // on their axes and V/m in their bins.
  ...
  if (spaceChargeScale == 0.0) return;

  // Preferred full 3D local-Cartesian correction map.
  if (m_exCorrection3D && m_eyCorrection3D && m_ezCorrection3D)
  {
    ex_vcm += spaceChargeScale * InterpolateCorrection3DVcm(m_exCorrection3D, r_cm, phi_rad, z_cm);
    ey_vcm += spaceChargeScale * InterpolateCorrection3DVcm(m_eyCorrection3D, r_cm, phi_rad, z_cm);
    ez_vcm += spaceChargeScale * InterpolateCorrection3DVcm(m_ezCorrection3D, r_cm, phi_rad, z_cm);
    return;
  }

  // Backward-compatible axisymmetric fallback.
  if (!m_erCorrection || !m_ezCorrection) return;
...
bool PHGarfield::LoadElectricFieldCorrections(const std::string& filename)
{
  ...
  {
    std::cerr << PHWHERE << " Could not open electric-field map: " << filename << std::endl;
    return false;
  }

  auto* ex3 = dynamic_cast<TH3*>(input->Get("Field3D/hEx"));
  auto* ey3 = dynamic_cast<TH3*>(input->Get("Field3D/hEy"));
  auto* ez3 = dynamic_cast<TH3*>(input->Get("Field3D/hEz"));
  if (!ex3) ex3 = dynamic_cast<TH3*>(input->Get("hEx"));
  if (!ey3) ey3 = dynamic_cast<TH3*>(input->Get("hEy"));
  if (!ez3) ez3 = dynamic_cast<TH3*>(input->Get("hEz"));
  ...
  if (ex3 && ey3 && ez3)
  {
    ...
    delete m_eyCorrection3D;
    delete m_ezCorrection3D;
    m_exCorrection3D = dynamic_cast<TH3*>(ex3->Clone("PHGarfield_ExCorrection3D"));
    m_eyCorrection3D = dynamic_cast<TH3*>(ey3->Clone("PHGarfield_EyCorrection3D"));
    m_ezCorrection3D = dynamic_cast<TH3*>(ez3->Clone("PHGarfield_EzCorrection3D"));
    if (!m_exCorrection3D || !m_eyCorrection3D || !m_ezCorrection3D)
    {
      delete m_exCorrection3D; m_exCorrection3D = nullptr;
      delete m_eyCorrection3D; m_eyCorrection3D = nullptr;
      delete m_ezCorrection3D; m_ezCorrection3D = nullptr;
      return false;
    }
    m_exCorrection3D->SetDirectory(nullptr);
    m_eyCorrection3D->SetDirectory(nullptr);
    m_ezCorrection3D->SetDirectory(nullptr);

    std::cout << "Loaded full 3D electric-field corrections from " << filename << std::endl;
    std::cout << "  axes: r [cm], phi [rad], signed z [cm]; bins: V/m" << std::endl;
    std::cout << "  r range [cm] = [" << m_exCorrection3D->GetXaxis()->GetXmin()
              << ", " << m_exCorrection3D->GetXaxis()->GetXmax() << "]" << std::endl;
    std::cout << "  phi range [rad] = [" << m_exCorrection3D->GetYaxis()->GetXmin()
              << ", " << m_exCorrection3D->GetYaxis()->GetXmax() << "]" << std::endl;
    std::cout << "  z range [cm] = [" << m_exCorrection3D->GetZaxis()->GetXmin()
              << ", " << m_exCorrection3D->GetZaxis()->GetXmax() << "]" << std::endl;
    return true;
  }
  ...
  if (!er || !ez)
  {
    std::cerr << PHWHERE << " Missing Field3D/hEx,hEy,hEz and legacy hErDefault,hEzDefault in "
              << filename << std::endl;
    return false;
  }
  ...
  if (!m_erCorrection || !m_ezCorrection) return false;
  m_erCorrection->SetDirectory(nullptr);
  m_ezCorrection->SetDirectory(nullptr);
  std::cout << "Loaded legacy axisymmetric electric-field corrections from " << filename << std::endl;
  return true;
}
    ...
    const double abs_z_cm) const
{
  ...
  const double r_eval = std::clamp(r_cm, r_min, r_max);
  const double z_eval = std::clamp(std::abs(abs_z_cm), z_min, z_max);
  return 0.01 * hist->Interpolate(r_eval, z_eval);
}

double PHGarfield::InterpolateCorrection3DVcm(
    const TH3* hist,
    const double r_cm,
    const double phi_rad,
    const double z_cm) const
{
  if (!hist) return 0.0;
  const TAxis* raxis = hist->GetXaxis();
  const TAxis* paxis = hist->GetYaxis();
  const TAxis* zaxis = hist->GetZaxis();
  if (!raxis || !paxis || !zaxis) return 0.0;

  const double r_eval = std::clamp(r_cm, raxis->GetBinCenter(1),
                                   raxis->GetBinCenter(raxis->GetNbins()));
  const double z_eval = std::clamp(z_cm, zaxis->GetBinCenter(1),
                                   zaxis->GetBinCenter(zaxis->GetNbins()));

  const double pmin = paxis->GetXmin();
  const double pmax = paxis->GetXmax();
  const double period = pmax - pmin;
  double p_eval = phi_rad;
  if (period > 0.0)
  {
    p_eval = pmin + std::fmod(phi_rad - pmin, period);
    if (p_eval < pmin) p_eval += period;
  }
  p_eval = std::clamp(p_eval, paxis->GetBinCenter(1),
                      paxis->GetBinCenter(paxis->GetNbins()));

  // ROOT TH3::Interpolate is trilinear. Convert V/m to V/cm.
  return 0.01 * hist->Interpolate(r_eval, p_eval, z_eval);
}
