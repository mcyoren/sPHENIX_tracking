...
class PHGarfield : public SubsysReco
{
 ...
 private:
  ...
  double InterpolateCorrection3DVcm(const TH3 *hist, double r_cm, double phi_rad, double z_cm) const;
  ...
  // Space-charge correction maps. Histograms are cloned and owned here.
  // Preferred format: Field3D/hEx, hEy, hEz as TH3(r,phi,z), bins in V/m.
  // Legacy fallback: QA/hErDefault and QA/hEzDefault as TH2(r,|z|).
  std::string m_electricFieldMap;
  double m_spaceChargeScale_side0{1.0};  // south, z < 0
  double m_spaceChargeScale_side1{1.0};  // north, z > 0
  double m_CMVoltageDefault{380.8};  // V/cm, nominal TPC field
  TH3 *m_exCorrection3D{nullptr};
  TH3 *m_eyCorrection3D{nullptr};
  TH3 *m_ezCorrection3D{nullptr};
  TH2 *m_erCorrection{nullptr};  // legacy radial correction, input bins in V/m
  TH2 *m_ezCorrection{nullptr};  // legacy local longitudinal correction, input bins in V/m
