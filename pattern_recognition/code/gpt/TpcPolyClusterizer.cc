...
#include </sphenix/user/mitrankov/garf/PHGarfield/install/include/phgarfield/PHGarfield.h>
...
namespace
{
  std::unique_ptr<PHGarfield> g_garfieldKeff10;
  unsigned int g_keffComparisonPrints = 0;
  constexpr unsigned int g_maxKeffComparisonPrints = 30;

  bool sample_drift_polyline(TPolyLine3D* drift,
                             const double target_time_ns,
                             ...
                             double& frac)
  {
    if (!drift || drift->GetN() <= 0 || step_ns <= 0.0) return false;

    const int npoints = drift->GetN();
    const Float_t* xyz = drift->GetP();
    if (!xyz) return false;

    const double max_time_ns = static_cast<double>(npoints - 1) * step_ns;
    if (target_time_ns < 0.0 || target_time_ns > max_time_ns) return false;

    const double fbin = target_time_ns / step_ns;
    i0 = std::min(static_cast<int>(std::floor(fbin)), npoints - 1);
    const int i1 = std::min(i0 + 1, npoints - 1);
    frac = fbin - static_cast<double>(i0);

    const int idx0 = 3 * i0;
    const int idx1 = 3 * i1;
    x = xyz[idx0] + frac * (xyz[idx1] - xyz[idx0]);
    y = xyz[idx0 + 1] + frac * (xyz[idx1 + 1] - xyz[idx0 + 1]);
    z = xyz[idx0 + 2] + frac * (xyz[idx1 + 2] - xyz[idx0 + 2]);
...
int TpcPolyClusterizer::InitRun(PHCompositeNode* topNode)
{
  ...
  delete m_garfield;
  m_garfield = nullptr;
  g_garfieldKeff10.reset();
  g_keffComparisonPrints = 0;
  
  const std::string electricFieldMap ="/sphenix/user/mitrankov/garf/include/sphenix_rossegger_garfield_field.root";

  // Main reconstruction instance: k_eff = 0.
  m_garfield = new PHGarfield(Name() + "_PHGarfield_keff0", electricFieldMap, 0.0);

  // Diagnostic instance: same map and initial point, but k_eff = 10.
  g_garfieldKeff10 = std::make_unique<PHGarfield>(
      Name() + "_PHGarfield_keff10", electricFieldMap, 10.0);

  // These PHGarfield objects are created by TpcPolyClusterizer rather than
  // registered directly with Fun4All. Therefore InitRun must be called
  // explicitly for each object so that the field maps, gas and Garfield
  // callbacks are initialized.
  if (m_garfield->InitRun(topNode) != Fun4AllReturnCodes::EVENT_OK)
  {
    std::cerr << Name() << "::InitRun - PHGarfield k_eff=0 InitRun failed" << std::endl;
    return Fun4AllReturnCodes::ABORTRUN;
  }

  if (g_garfieldKeff10->InitRun(topNode) != Fun4AllReturnCodes::EVENT_OK)
  {
    std::cerr << Name() << "::InitRun - PHGarfield k_eff=10 InitRun failed" << std::endl;
    return Fun4AllReturnCodes::ABORTRUN;
  }

  std::cout << Name()
            << "::InitRun - enabled PHGarfield comparison"
            << " map=" << electricFieldMap
            << " keff0=" << m_garfield->GetSpaceChargeScale()
            << " keff10=" << g_garfieldKeff10->GetSpaceChargeScale()
            << " ptr0=" << static_cast<const void*>(m_garfield)
            << " ptr10=" << static_cast<const void*>(g_garfieldKeff10.get())
            << std::endl;

  // Fixed-point startup diagnostic. This runs before processing hits and
  // verifies that the two fully initialized objects return different sampled
  // coordinates for exactly the same start point and drift time.
  {
    constexpr double test_x0 = 0.0;
    constexpr double test_y0 = 40.0;
    constexpr double test_z0 = -102.0;
    constexpr double test_time_ns = 10000.0;

    std::unique_ptr<TPolyLine3D> test_drift0(
        m_garfield->ReverseDrift(test_x0, test_y0, test_z0, m_reverseDriftStepNs));
    std::unique_ptr<TPolyLine3D> test_drift10(
        g_garfieldKeff10->ReverseDrift(test_x0, test_y0, test_z0, m_reverseDriftStepNs));
    ...
    const bool ok0 = sample_drift_polyline(
        test_drift0.get(), test_time_ns, m_reverseDriftStepNs,
        x0, y0, z0, idx0, frac0);
    const bool ok10 = sample_drift_polyline(
        test_drift10.get(), test_time_ns, m_reverseDriftStepNs,
        x10, y10, z10, idx10, frac10);

    if (ok0 && ok10)
    {
      const double dx = x10 - x0;
      const double dy = y10 - y0;
      const double dz = z10 - z0;
      const double drphi = std::hypot(dx, dy);
      const double d3 = std::sqrt(dx * dx + dy * dy + dz * dz);

      std::cout << Name() << "::PHGarfieldKeffInitTest"
                << " start=(" << test_x0 << "," << test_y0 << "," << test_z0 << ")"
                << " target_ns=" << test_time_ns
                << " keff0=(" << x0 << "," << y0 << "," << z0 << ")"
                << " keff10=(" << x10 << "," << y10 << "," << z10 << ")"
                << " delta=(" << dx << "," << dy << "," << dz << ")"
                << " delta_rphi=" << drphi
    ...
    {
      std::cerr << Name() << "::PHGarfieldKeffInitTest - failed to sample"
                << " ok0=" << ok0 << " ok10=" << ok10
                << std::endl;
    }
  }
                                        ...
                                        Point& p) const
{
  ...
  std::unique_ptr<TPolyLine3D> drift0(
      m_garfield->ReverseDrift(x0, y0, z0, m_reverseDriftStepNs));

  double x = 0.0;
  double y = 0.0;
  double z = 0.0;
  int sampleIndex0 = -1;
  double sampleFrac0 = 0.0;

  if (!sample_drift_polyline(drift0.get(), target_time_ns, m_reverseDriftStepNs,
                             x, y, z, sampleIndex0, sampleFrac0))
  {
    return false;
  }

  // Diagnostic only: evaluate exactly the same hit and time using k_eff=10.
  // The k_eff=0 coordinates above remain the ones used by reconstruction.
  if (g_garfieldKeff10 && g_keffComparisonPrints < g_maxKeffComparisonPrints)
  {
    std::unique_ptr<TPolyLine3D> drift10(
        g_garfieldKeff10->ReverseDrift(x0, y0, z0, m_reverseDriftStepNs));

    double x10 = 0.0;
    double y10 = 0.0;
    double z10 = 0.0;
    int sampleIndex10 = -1;
    double sampleFrac10 = 0.0;

    const bool ok10 = sample_drift_polyline(
        drift10.get(), target_time_ns, m_reverseDriftStepNs,
        x10, y10, z10, sampleIndex10, sampleFrac10);

    if (ok10)
    {
      const double dx = x10 - x;
      const double dy = y10 - y;
      const double dz = z10 - z;
      const double drphi = std::hypot(dx, dy);
      const double d3 = std::sqrt(dx * dx + dy * dy + dz * dz);

      std::cout << Name() << "::PHGarfieldKeffTest"
                << " event=" << m_event
                << " layer=" << layer
                << " side=" << side
                << " pad=" << pad
                << " tbin=" << tbin
                << " target_ns=" << target_time_ns
                << " start=(" << x0 << "," << y0 << "," << z0 << ")"
                << " keff0=(" << x << "," << y << "," << z << ")"
                << " keff10=(" << x10 << "," << y10 << "," << z10 << ")"
                << " delta=(" << dx << "," << dy << "," << dz << ")"
                << " delta_rphi=" << drphi
                << " delta_3d=" << d3
                << " n0=" << (drift0 ? drift0->GetN() : 0)
                << " n10=" << (drift10 ? drift10->GetN() : 0)
                << " idx0=" << sampleIndex0
                << " idx10=" << sampleIndex10
                << std::endl;
    }
    else
    {
      std::cout << Name() << "::PHGarfieldKeffTest"
                ...
                << " pad=" << pad
                << " tbin=" << tbin
                << " k_eff=10 polyline could not be sampled"
                << std::endl;
    }

    ++g_keffComparisonPrints;
  }
