...
namespace
{
  ...
  // Evaluate sagitta phi and its analytic derivative dphi/dr from the
  // same Newton solution.  The old code evaluated phi once and then
  // solved the sagitta equation two more times at r +/- eps to obtain
  // the slope numerically.
  //
  // With
  //   xrot = c*r + s*y
  //   yrot = -s*r + c*y
  // and yrot = f(xrot), implicit differentiation gives
  //
  //   dy/dr = (s + f'(xrot)*c) / (c - f'(xrot)*s).
  //
  // Since predicted phi is bline + y, this is also dphi/dr.
  bool predict_sagitta_phi_and_slope(double radius,
                                     double S,
                                     double x0,
                                     double invR,
                                     double theta,
                                     ...
                                     double& slope)
  {
    ...
    {
      // Extremely rare singular/near-singular case: preserve the old
      // finite-difference behavior as a safe fallback.
      constexpr double eps = 1.0e-3;
      slope =
          (predict_sagitta_phi(radius + eps, S, x0, invR, theta, bline) -
           predict_sagitta_phi(radius - eps, S, x0, invR, theta, bline)) /
          (2.0 * eps);
    }

    return std::isfinite(phi) && std::isfinite(slope);
  }
...
{
  double phi = 0.0;
  double slope = p.phi_slope;
  if (!predict_phi_and_slope(p, radius, phi, slope))
  {
    return p.phi_slope;
  }
  return slope;
...
{
  double phi = 0.0;
  double slope = c.phi_slope;
  if (!predict_phi_and_slope(c, radius, phi, slope))
  {
    return c.phi_slope;
  }
  return slope;
...
bool Tpc_AssembledTrackReco::predict_phi_and_slope(const Piece& p,
                                                   double radius,
                                                   double& phi,
                                                   double& slope) const
{
  ...
  phi = p.phi_slope * radius + p.phi_intercept;
  slope = p.phi_slope;
  return std::isfinite(phi) && std::isfinite(slope);
}

bool Tpc_AssembledTrackReco::predict_phi_and_slope(const Candidate& c,
                                                   double radius,
                                                   double& phi,
                                                   double& slope) const
{
  ...
  phi = c.phi_slope * radius + c.phi_intercept;
  slope = c.phi_slope;
  return std::isfinite(phi) && std::isfinite(slope);
}
                                                      ...
                                                      double& b_phi_intercept_shifted) const
{
  ...
  // Evaluate phi and dphi/dr together so each object needs only one
  // sagitta/Newton solve.
  double phi_a = 0.0;
  double slope_a = 0.0;
  double phi_b_raw = 0.0;
  double slope_b = 0.0;

  if (!predict_phi_and_slope(a, rmatch, phi_a, slope_a) ||
      !predict_phi_and_slope(b, rmatch, phi_b_raw, slope_b))
  {
    return false;
  }
  ...
  const double dmphi = std::fabs(slope_a - slope_b);
                                                      ...
                                                      double& score) const
{
  ...
  // Evaluate phi and dphi/dr together so each candidate needs only one
  // sagitta/Newton solve.
  double phi_a = 0.0;
  double slope_a = 0.0;
  double phi_b_raw = 0.0;
  ...
  const double phi_b =
      unwrap_phi_to_reference(phi_b_raw, phi_a);
...
Tpc_AssembledTrackReco::make_seed_parameters(const Candidate& c) const
{
  ...
  double dphi_dr = 0.0;
  if (!predict_phi_and_slope(c, radius, predicted_phi, dphi_dr))
  {
    return seed;
  }

  const double phi = wrap_to_pi(predicted_phi);
