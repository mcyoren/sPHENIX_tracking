...
class Tpc_AssembledTrackReco : public SubsysReco
{
 public:
  ...
  void setDoTiming(bool v) { m_doTiming = v; }
 ...
 private:
  ...
  // Optional lightweight performance counters. Disabled by default.
  mutable std::atomic<unsigned long long> m_timingMakePieceNs{0};
  mutable std::atomic<unsigned long long> m_timingSectorNs{0};
  mutable std::atomic<unsigned long long> m_timingSideNs{0};
  mutable std::atomic<unsigned long long> m_timingRefitNs{0};

  mutable std::atomic<unsigned long long> m_timingMakePieceCalls{0};
  mutable std::atomic<unsigned long long> m_timingRefitCalls{0};

  mutable std::atomic<unsigned long long> m_timingPiecePairTests{0};
  mutable std::atomic<unsigned long long> m_timingPiecePassTbin{0};
  mutable std::atomic<unsigned long long> m_timingPiecePassLinearPhi{0};
  mutable std::atomic<unsigned long long> m_timingPiecePassExact{0};

  mutable std::atomic<unsigned long long> m_timingCandidatePairTests{0};
  mutable std::atomic<unsigned long long> m_timingCandidatePassTbin{0};
  mutable std::atomic<unsigned long long> m_timingCandidatePassLinearPhi{0};
  mutable std::atomic<unsigned long long> m_timingCandidatePassExact{0};
