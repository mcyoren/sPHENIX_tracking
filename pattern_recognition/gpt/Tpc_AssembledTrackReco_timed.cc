...
void Tpc_AssembledTrackReco::connect_sector_pieces(const std::vector<Piece>& pieces, int side, unsigned int sector, std::vector<Candidate>& output) const
{
  ...
  if (m_doTiming)
  {
    const auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::steady_clock::now() - timing_start).count();
    m_timingSectorNs.fetch_add(static_cast<unsigned long long>(ns), std::memory_order_relaxed);
  }
}
                                                     ...
                                                     std::vector<Candidate>& output) const
{
  ...
  if (m_doTiming)
  {
    const auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::steady_clock::now() - timing_start).count();
    m_timingSideNs.fetch_add(static_cast<unsigned long long>(ns), std::memory_order_relaxed);
  }
}
