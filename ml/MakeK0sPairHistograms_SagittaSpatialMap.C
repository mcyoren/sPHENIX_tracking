// MakeK0sPairHistograms_SagittaSpatialMap.C
//
// One-pass V0 QA for TpcDstV0Finder pairTree.
//
// Optional six-radius sagitta spatial correction:
//   applySagittaSpatialMap=true loads side-dependent A(phi,z) maps,
//   propagates each daughter from its production vertex to six TPC radii,
//   applies rDeltaPhi=A(phi,z)*g_sag(r), refits the transverse circle,
//   recalculates pT and tangent phi, and then recalculates all masses and
//   pair-pT values. The same spatial map is used for both charges.
...
// Example:
// root -l -b -q \
// 'MakeK0sPairHistograms_SagittaSpatialMap.C("/path/to/V0/files","HITS*_V0.root","output","v0_qa.root","pairTree",true)'
  ...
  class SagittaSpatialMap
  {
   public:
    bool load(const char* fileName)
    {
      ...
      {
        std::cerr
          << "ERROR: sagitta spatial-map filename is empty"
          << std::endl;
        return false;
      }
      ...
      {
        std::cerr
          << "ERROR: cannot open sagitta spatial map "
          << fileName << std::endl;
        return false;
      }
      ...
      for (int side = 0; side < 2; ++side)
      {
        const TString path = TString::Format(
          "side%d/h2_sagitta_amplitude",
          side);

        TH2D* source =
          dynamic_cast<TH2D*>(m_file->Get(path));

        if (!source)
        {
          std::cerr
            << "ERROR: missing sagitta-amplitude histogram "
            << path << std::endl;
          return false;
        }

        m_amplitudeMaps[side] =
          dynamic_cast<TH2D*>(source->Clone(
            TString::Format(
              "h2_sagitta_amplitude_side%d_local",
              side)));

        if (!m_amplitudeMaps[side])
        {
          std::cerr
        ...
        m_amplitudeMaps[side]->SetDirectory(nullptr);
      }
    ...
    CorrectedTrack correct(
      ...
      const double vertexZ) const
    {
      CorrectedTrack result;
      ...
      if (!m_loaded ||
          ...
          !std::isfinite(py) ||
          !std::isfinite(pz) ||
          !std::isfinite(vertexX) ||
          !std::isfinite(vertexY) ||
          !std::isfinite(vertexZ))
      {
        return result;
      }
      ...
      std::array<Point, kNumberOfPseudoRadii> points;

      for (std::size_t index = 0;
           index < kNumberOfPseudoRadii;
           ++index)
      {
        const double radius = kPseudoRadii[index];

        Point point;

        if (!propagateToRadius(
              vertexX,
              vertexY,
              vertexZ,
              phi0,
        ...
        const double radialBasis =
          sagittaRadialBasis(radius);

        const double rDeltaPhi =
          amplitude * radialBasis;
        ...
        const double correctedPhi =
          std::atan2(point.y, point.x) +
          rDeltaPhi / pointRadius;

        point.x =
          pointRadius * std::cos(correctedPhi);

        point.y =
          pointRadius * std::sin(correctedPhi);
      ...
      // The propagation convention is
      // d(phi_momentum)/dsT = -q/R.
      const double fittedOmega =
        -charge / fittedRadius;
      ...
      const double correctedPhi =
        std::atan2(sinPhi, cosPhi);

      const double correctedPt =
        kCurvatureConversion *
        kMagneticFieldTesla *
        fittedRadius;

      if (!(correctedPt > 0.0) ||
          !std::isfinite(correctedPhi))
      {
        return result;
      }

      result.valid = true;
      result.pt = correctedPt;
      result.phi = correctedPhi;
      result.eta = eta;
      result.px = correctedPt * std::cos(correctedPhi);
      result.py = correctedPt * std::sin(correctedPhi);
      result.pz = correctedPt * std::sinh(eta);
      result.momentumScale = correctedPt / pt;
      result.phiShift = wrapPhiLocal(correctedPhi - phi0);
   ...
   private:
    ...
    static constexpr double kMagneticFieldTesla = 1.4;
    static constexpr double kCurvatureConversion = 0.003;

    static constexpr std::size_t kNumberOfPseudoRadii = 6;

    static constexpr std::array<double, kNumberOfPseudoRadii>
      kPseudoRadii = {
        33.0,
        41.0,
        49.0,
    ...
    static constexpr double kInnerRadius = 33.0;
    static constexpr double kOuterRadius = 73.0;

    static double wrapPhiLocal(double phi)
    {
    ...
    static double sagittaRadialBasis(
      const double radius)
    {
      ...
      Point& result)
    {
      ...
      const double centerX =
        x0 - std::sin(phi0) / omega;

      const double centerY =
        y0 + std::cos(phi0) / omega;

      const double trackRadius =
        std::abs(1.0 / omega);

      const double centerDistance =
        std::hypot(centerX, centerY);

      if (!(centerDistance > 1.0e-9) ||
          targetRadius >
            centerDistance + trackRadius ||
          targetRadius <
            std::abs(centerDistance - trackRadius))
      {
        return false;
      }
      ...
      if (heightSquared < -1.0e-8)
      {
        return false;
      }

      heightSquared =
        std::max(0.0, heightSquared);

      const double height =
        std::sqrt(heightSquared);

      const double unitX =
        centerX / centerDistance;

      const double unitY =
        centerY / centerDistance;
      ...
      const double perpendicularX =
        -unitY;

      const double perpendicularY =
        unitX;

      const Point intersectionA{
        baseX + height * perpendicularX,
        baseY + height * perpendicularY,
        0.0};

      const Point intersectionB{
        baseX - height * perpendicularX,
        baseY - height * perpendicularY,
        0.0};

      const auto tangentPhi =
        [omega, centerX, centerY](
          const Point& point)
      {
        const double sinPhi =
          omega * (point.x - centerX);

        const double cosPhi =
          -omega * (point.y - centerY);

        return std::atan2(sinPhi, cosPhi);
      };

      const auto forwardPath =
        [omega, phi0](
          const double targetPhi)
      {
        double deltaPhi = 0.0;
      ...
      const double pathA =
        forwardPath(tangentPhi(intersectionA));

      const double pathB =
        forwardPath(tangentPhi(intersectionB));
      ...
      result =
        chooseA ? intersectionA : intersectionB;

      result.z =
        z0 + path * std::sinh(eta);
      ...
      double solution[3])
    {
           ...
           ++pivot)
      {
        ...
        if (!(std::abs(matrix[bestRow][pivot]) >
              1.0e-14))
        {
          return false;
        }
      ...
      double& radius)
    {
      ...
      double matrix[3][3] = {
        ...
        {sumX, sumY,
         static_cast<double>(
           kNumberOfPseudoRadii)}
      };

      // Small diagonal ridge, matching the notebook's
      // differentiable normal-equation fit.
      for (int index = 0;
           index < 3;
           ++index)
      {
...
void MakeK0sPairHistograms_SagittaSpatialMap(
  const char* inputDir = ".",
  ...
  const bool applySagittaSpatialMap = true,
  const char* sagittaSpatialMapFile =
    "input/k0s_sagitta_spatial_map.root")
{
  TH1::AddDirectory(kTRUE);

  std::cout << "MakeK0sPairHistograms_SagittaSpatialMap: inputDir=" << inputDir
            ...
            << ", beamY=" << beamY
            << ", beamZ=" << beamZ
            << ", applySagittaSpatialMap=" << applySagittaSpatialMap
            << ", sagittaSpatialMapFile=" << sagittaSpatialMapFile
            << std::endl;
  ...
  SagittaSpatialMap sagittaSpatialMap;

  if (applySagittaSpatialMap &&
      !sagittaSpatialMap.load(sagittaSpatialMapFile))
  {
    return;
  }
  ...
  TNamed sagittaSpatialInfo(
    "sagitta_spatial_correction",
    applySagittaSpatialMap
      ? TString::Format(
          "enabled; map=%s; six-radius rDeltaPhi correction, circle refit, "
          "and tangent-phi recalculation applied to secondary and prompt daughters",
          sagittaSpatialMapFile).Data()
      : "disabled");
  sagittaSpatialInfo.Write();
  ...
  std::cout
    ...
    << (usePrimaryVertexKinematicsForPrompt
          ...
          : "secondary pair PCA")
    << ", sagitta spatial map = "
    << (applySagittaSpatialMap ? sagittaSpatialMapFile : "disabled")
    << std::endl;

  Long64_t secondaryCorrectionFailures = 0;
  Long64_t promptCorrectionFailures = 0;
       ...
       ++entry)
  {
    ...
    // Both daughters are propagated from the reconstructed
    // secondary vertex through the common spatial correction.
    // --------------------------------------------------------
    ...
    CorrectedTrack secondaryCorrection1;
    CorrectedTrack secondaryCorrection2;

    if (applySagittaSpatialMap)
    {
      secondaryCorrection1 =
        sagittaSpatialMap.correct(
          secondarySide1,
          charge1,
          px1,
          py1,
      ...
      if (!secondaryCorrection1.valid)
      {
        ++secondaryCorrectionFailures;
      }

      if (!secondaryCorrection2.valid)
      {
        ++secondaryCorrectionFailures;
      }
    }

    const bool useSecondaryCorrection1 =
      applySagittaSpatialMap &&
      secondaryCorrection1.valid;

    const bool useSecondaryCorrection2 =
      applySagittaSpatialMap &&
      secondaryCorrection2.valid;

    const double secondaryPx1 =
      useSecondaryCorrection1
        ? secondaryCorrection1.px
        : px1;

    const double secondaryPy1 =
      useSecondaryCorrection1
        ? secondaryCorrection1.py
        : py1;

    const double secondaryPz1 =
      useSecondaryCorrection1
        ? secondaryCorrection1.pz
        : pz1;

    const double secondaryPx2 =
      useSecondaryCorrection2
        ? secondaryCorrection2.px
        : px2;

    const double secondaryPy2 =
      useSecondaryCorrection2
        ? secondaryCorrection2.py
        : py2;

    const double secondaryPz2 =
      useSecondaryCorrection2
        ? secondaryCorrection2.pz
        : pz2;
    ...
    // --------------------------------------------------------
    // Prompt kinematics.
    // For primary-PCA momenta, each track starts from its own
    // beam-axis PCA: (beamX,beamY,primary_pca_z).
    // When secondary kinematics are requested, both tracks start
    // from the reconstructed pair PCA.
    // --------------------------------------------------------
    ...
    const double promptVertexX1 =
      usePrimaryVertexKinematicsForPrompt ? beamX : pca_x;
    const double promptVertexY1 =
      usePrimaryVertexKinematicsForPrompt ? beamY : pca_y;
    const double promptVertexZ1 =
      usePrimaryVertexKinematicsForPrompt ? primary_pca1_z : pca_z;

    const double promptVertexX2 =
      usePrimaryVertexKinematicsForPrompt ? beamX : pca_x;
    const double promptVertexY2 =
      usePrimaryVertexKinematicsForPrompt ? beamY : pca_y;
    const double promptVertexZ2 =
      usePrimaryVertexKinematicsForPrompt ? primary_pca2_z : pca_z;
    ...
    CorrectedTrack promptCorrection1;
    CorrectedTrack promptCorrection2;
    ...
    if (applySagittaSpatialMap)
    {
      ...
      if (!promptCorrection1.valid)
      {
        ++promptCorrectionFailures;
      }

      if (!promptCorrection2.valid)
      {
        ++promptCorrectionFailures;
      }
    }

    const bool usePromptCorrection1 =
      applySagittaSpatialMap &&
      promptCorrection1.valid;

    const bool usePromptCorrection2 =
      applySagittaSpatialMap &&
      promptCorrection2.valid;

    const double promptPx1 =
      usePromptCorrection1
        ? promptCorrection1.px
        : rawPromptPx1;

    const double promptPy1 =
      usePromptCorrection1
        ? promptCorrection1.py
        : rawPromptPy1;

    const double promptPz1 =
      usePromptCorrection1
        ? promptCorrection1.pz
        : rawPromptPz1;

    const double promptPx2 =
      usePromptCorrection2
        ? promptCorrection2.px
        : rawPromptPx2;

    const double promptPy2 =
      usePromptCorrection2
        ? promptCorrection2.py
        : rawPromptPy2;

    const double promptPz2 =
      usePromptCorrection2
        ? promptCorrection2.pz
        : rawPromptPz2;
    ...
    const double dira =
      (v0Momentum > 0. && flightLength > 0.)
        ? (correctedV0Px * flightX +
           correctedV0Py * flightY +
           correctedV0Pz * flightZ) /
  ...
  {
    std::cout
      << "Sagitta correction fallbacks: secondary tracks = "
      << secondaryCorrectionFailures
      << ", prompt tracks = "
      << promptCorrectionFailures
      << std::endl;
