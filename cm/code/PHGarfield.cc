...
int PHGarfield::InitRun(PHCompositeNode* /*topNode*/)
{
  ...
  // Precompute the axisymmetric IFC boundary field once.  The stored grid is
  // for a +1 V endpoint perturbation and is scaled independently for South
  // and North at field-query time.
  if (m_useIFCVoltageDistortion &&
      (m_ifcVoltageOffset_side0 != 0.0 || m_ifcVoltageOffset_side1 != 0.0) &&
      !BuildIFCVoltageFieldGrid())
  {
    std::cerr << PHWHERE << " Failed to build IFC boundary-voltage field grid; "
              << "IFC correction will be disabled." << std::endl;
...
void PHGarfield::GetElectricFieldVcm(double x_cm, double y_cm, double z_cm, double& ex_vcm, double& ey_vcm, double& ez_vcm) const
{
  ...
  if (spaceChargeScale == 0.0)
  {
    ...
    AddIFCVoltageDistortion(x_cm, y_cm, z_cm, ex_vcm, ey_vcm, ez_vcm);
    return;
  }
...
bool PHGarfield::BuildIFCVoltageFieldGrid()
{
  m_ifcGridReady = false;
  ...
  {
    std::cerr << PHWHERE << " Invalid IFC boundary solver configuration." << std::endl;
    return false;
  }
  ...
  // Precompute radial eigenfunctions and their radial derivatives.  This keeps
  // the expensive modified-Bessel evaluations out of GetElectricFieldVcm().
  std::vector<double> radial(static_cast<std::size_t>(m_ifcVoltageModes) * m_ifcGridNR, 0.0);
  std::vector<double> dradial(static_cast<std::size_t>(m_ifcVoltageModes) * m_ifcGridNR, 0.0);
  ...
  for (unsigned int n = 1; n <= m_ifcVoltageModes; ++n)
  {
    ...
    const double denom = k0b * i0a - i0b * k0a;

    if (!std::isfinite(denom) || denom == 0.0)
    {
      std::cerr << PHWHERE << " Invalid IFC radial mode n=" << n << std::endl;
      return false;
    }
    ...
    for (unsigned int ir = 0; ir < m_ifcGridNR; ++ir)
    {
      ...
      radial[idx] = (k0b * i0r - i0b * k0r) / denom;
      dradial[idx] = k * (k0b * i1r + i0b * k1r) / denom;
    }
  }

  // +1 V endpoint perturbation. For dV(a,s)=s/L, the sine coefficient is
  // b_n = 2*(-1)^(n+1)/(n*pi).
  for (unsigned int iz = 0; iz < m_ifcGridNZ; ++iz)
  {
    ...
    for (unsigned int ir = 0; ir < m_ifcGridNR; ++ir)
    {
      ...
      for (unsigned int n = 1; n <= m_ifcVoltageModes; ++n)
      {
        ...
        er -= bn * dradial[ridx] * std::sin(phase);
        es -= bn * radial[ridx] * k * std::cos(phase);
      }

      const std::size_t gidx = static_cast<std::size_t>(iz) * m_ifcGridNR + ir;
      m_ifcUnitErGrid[gidx] = er;
      m_ifcUnitEsGrid[gidx] = es;
    }
  }
  ...
  if (Verbosity() > 0)
  {
    std::cout << "PHGarfield: built IFC boundary field grid: "
              << m_ifcGridNR << " x " << m_ifcGridNZ
              << ", modes=" << m_ifcVoltageModes
              << ", r=[" << a << ", " << b << "] cm"
              << ", |z|=[0, " << L << "] cm"
              << ", dV endpoint South/North = ("
              << m_ifcVoltageOffset_side0 << ", "
              << m_ifcVoltageOffset_side1 << ") V" << std::endl;
  }
  return true;
}

double PHGarfield::InterpolateIFCGrid(const std::vector<double>& grid,
                                      const double r_cm,
                                      const double abs_z_cm) const
{
  if (!m_ifcGridReady || grid.empty() || m_ifcGridNR < 2 || m_ifcGridNZ < 2 ||
...
// Add the static field from the changed IFC boundary condition. This is kept
// independent of the Rossegger volume-space-charge correction.
void PHGarfield::AddIFCVoltageDistortion(const double x_cm,
                                         const double y_cm,
                                         const double z_cm,
                                         double& ex_vcm,
                                         double& ey_vcm,
                                         double& ez_vcm) const
{
  if (!m_useIFCVoltageDistortion || !m_ifcGridReady)
  {
    return;
  }

  const std::size_t side = z_cm < 0.0 ? 0 : 1;
  const double delta_v_end = side == 0 ? m_ifcVoltageOffset_side0
                                        : m_ifcVoltageOffset_side1;
  if (delta_v_end == 0.0)
  {
    return;
  }
  ...
  if (r < m_ifcInnerRadius_cm || r > m_ifcOuterRadius_cm ||
      s < 0.0 || s > m_ifcHalfLength_cm)
  {
    return;
  }

  const double delta_er_vcm = delta_v_end * InterpolateIFCGrid(m_ifcUnitErGrid, r, s);
  const double delta_es_vcm = delta_v_end * InterpolateIFCGrid(m_ifcUnitEsGrid, r, s);
