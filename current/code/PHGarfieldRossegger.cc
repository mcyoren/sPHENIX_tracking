...
namespace
{
  ...
  struct ModuleFrameGeometry
  {
    FrameContour outer_boundary;  // selected L49 contour from the drawing
    FrameContour inner_boundary;  // selected L101 contour from the drawing
    double area_m2{0.0};
  };

  constexpr unsigned int frameReferenceSector = 2;
  constexpr std::array<double, nTpcModules> frameOriginXMm{{5.5057009742769056e-05, 0.0, 0.0}};
  constexpr std::array<double, nTpcModules> frameOriginYMm{{3.1752057245087677, 3.254692499999908, 3.175029592886644}};

  std::vector<DrawingWire> frame_drawing_wires(unsigned int module, bool outer_boundary)
  {
    if (module == 0 && outer_boundary)
    {
      return {
          {95.53156875, 390.731140625, 97.05715, 379.10755, 0},
          {97.05715, 379.10755, 97.040909375, 378.86056875, -22.48},
          {97.040909375, 378.86056875, 54.994690625, 221.94193125, 0},
          {54.994690625, 221.94193125, 54.88706875, 221.72228125, -22.21},
          {54.88706875, 221.72228125, 50.877940625, 216.44215, 0},
          {50.877940625, 216.44215, 50.228509375, 216.20761875, -65.87},
          {50.228509375, 216.20761875, -50.228509375, 216.20761875, 26.16},
          {-50.228509375, 216.20761875, -50.877940625, 216.44215, -65.87},
          {-50.877940625, 216.44215, -52.207709375, 218.193490625, 0},
          {-52.207709375, 218.193490625, -52.31533125, 218.413140625, -22.21},
          {-52.31533125, 218.413140625, -96.470859375, 383.20383125, 0},
          {-96.470859375, 383.20383125, -96.4871, 383.45081875, -22.48},
          {-96.4871, 383.45081875, -95.53156875, 390.731140625, 0},
          {-95.53156875, 390.731140625, -95.05186875, 391.265559375, -68.87},
          {-95.05186875, 391.265559375, 95.05186875, 391.265559375, -27.31},
          {95.05186875, 391.265559375, 95.53156875, 390.731140625, -68.87}};
    }
    if (module == 0)
    {
      return {
          {-52.18043125, 249.69731875, -44.902490625, 245.09476875, 85.38},
          {-44.902490625, 245.09476875, 44.902490625, 245.09476875, -20.76},
          {44.902490625, 245.09476875, 52.18043125, 249.69731875, 85.38},
          {52.18043125, 249.69731875, 86.175209375, 376.567590625, 0},
          {86.175209375, 376.567590625, 81.35633125, 384.423490625, 93.05},
          {81.35633125, 384.423490625, -81.35633125, 384.423490625, 23.9},
          {-81.35633125, 384.423490625, -86.175209375, 376.567590625, 93.05},
          {-86.175209375, 376.567590625, -52.18043125, 249.69731875, 0}};
    }
    if (module == 1 && outer_boundary)
    {
      return {
          {-102.40388125, 405.34615, -141.266940625, 550.38506875, 0},
          {-141.266940625, 550.38506875, -141.279040625, 550.65905, -24.94},
          {-141.279040625, 550.65905, -140.136140625, 557.17953125, 0},
          {-140.136140625, 557.17953125, -139.664940625, 557.68588125, -66},
          {-139.664940625, 557.68588125, 139.664940625, 557.68588125, -28.12},
          {139.664940625, 557.68588125, 140.136140625, 557.17953125, -66},
          {140.136140625, 557.17953125, 141.9647, 546.74723125, 0},
          {141.9647, 546.74723125, 141.9526, 546.47325, -24.94},
          {141.9526, 546.47325, 104.96266875, 408.424959375, 0},
          {104.96266875, 408.424959375, 104.837659375, 408.183440625, -24.73},
          {104.837659375, 408.183440625, 97.97721875, 399.928759375, 0},
          {97.97721875, 399.928759375, 97.33861875, 399.717659375, -63.96},
          {97.33861875, 399.717659375, -97.33861875, 399.717659375, 27.37},
          {-97.33861875, 399.717659375, -97.97721875, 399.928759375, -63.96},
          {-97.97721875, 399.928759375, -102.27888125, 405.10463125, 0},
          {-102.27888125, 405.10463125, -102.40388125, 405.34615, -24.73}};
    }
    if (module == 1)
    {
      return {
          {-96.838759375, 416.36446875, -89.35855, 411.802390625, 87.24},
          {-89.35855, 411.802390625, 89.35855, 411.802390625, -24.49},
          {89.35855, 411.802390625, 96.838759375, 416.36446875, 87.24},
          {96.838759375, 416.36446875, 130.73806875, 542.87841875, 0},
          {130.73806875, 542.87841875, 126.020909375, 550.71191875, 92.11},
          {126.020909375, 550.71191875, -126.020909375, 550.71191875, 25.78},
          {-126.020909375, 550.71191875, -130.73806875, 542.87841875, 92.11},
          {-130.73806875, 542.87841875, -96.838759375, 416.36446875, 0}};
    }
    if (outer_boundary)
    {
      return {
          {-189.43793125, 731.12756875, -146.57755, 571.170440625, 0},
          {-146.57755, 571.170440625, -146.45228125, 570.9286, 24.77},
          {-146.45228125, 570.9286, -142.541140625, 566.22871875, 0},
          {-142.541140625, 566.22871875, -141.89863125, 566.01896875, 64.31},
          {-141.89863125, 566.01896875, 141.89863125, 566.01896875, -28.15},
          {141.89863125, 566.01896875, 142.541140625, 566.22871875, 64.31},
          {142.541140625, 566.22871875, 146.45228125, 570.9286, 0},
          {146.45228125, 570.9286, 146.57755, 571.170440625, 24.77},
          {146.57755, 571.170440625, 189.43793125, 731.12756875, 0},
          {189.43793125, 731.12756875, 189.45913125, 731.31536875, 17.12},
          {189.45913125, 731.31536875, 189.3643, 733.88171875, 0},
          {189.3643, 733.88171875, 188.887890625, 734.473259375, 73.46},
          {188.887890625, 734.473259375, -188.887890625, 734.473259375, 28.84},
          {-188.887890625, 734.473259375, -189.3643, 733.88171875, 73.46},
          {-189.3643, 733.88171875, -189.45913125, 731.31536875, 0},
          {-189.45913125, 731.31536875, -189.43793125, 731.12756875, 0}};
    }
    return {
        {-141.38153125, 582.60036875, -175.31978125, 709.259659375, 0},
        {-175.31978125, 709.259659375, -170.65631875, 717.08063125, -91.61},
        {-170.65631875, 717.08063125, 170.65631875, 717.08063125, -26.77},
        {170.65631875, 717.08063125, 175.31978125, 709.259659375, -91.61},
        {175.31978125, 709.259659375, 141.38153125, 582.60036875, 0},
        {141.38153125, 582.60036875, 133.8158, 578.05746875, -88.03},
        {133.8158, 578.05746875, -133.8158, 578.05746875, 26.07},
        {-133.8158, 578.05746875, -141.38153125, 582.60036875, -88.03}};
  }

  void append_drawing_arc(std::vector<std::pair<double, double>>& points,
                          double x1, double y1, double x2, double y2,
                          double angle_deg, unsigned int npts = 64)
  {
    const double dx = x2 - x1;
    const double dy = y2 - y1;
    const double half_chord = std::sqrt(dx * dx + dy * dy) / 2.0;
    const double theta = angle_deg * std::numbers::pi / 180.0;
    const double radius = half_chord / std::sin(std::fabs(theta) / 2.0);
    const double len = 2.0 * half_chord;
    const double px = -dy / len;
    const double py = dx / len;
    const double d = radius * std::cos(std::fabs(theta) / 2.0);
    const double sign = angle_deg >= 0.0 ? 1.0 : -1.0;
    const double cx = 0.5 * (x1 + x2) + sign * d * px;
    const double cy = 0.5 * (y1 + y2) + sign * d * py;
    double a0 = std::atan2(y1 - cy, x1 - cx);
    double a1 = std::atan2(y2 - cy, x2 - cx);

    if (angle_deg >= 0.0)
    {
      if (a1 < a0) { a1 += 2.0 * std::numbers::pi; }
    }
    else if (a1 > a0)
    {
      a1 -= 2.0 * std::numbers::pi;
    }

    for (unsigned int i = 0; i < npts; ++i)
    {
      const double t = a0 + (a1 - a0) * static_cast<double>(i) / static_cast<double>(npts);
      points.emplace_back(cx + radius * std::cos(t), cy + radius * std::sin(t));
    }
  }

  std::vector<std::pair<double, double>> rasterize_drawing_wires(const std::vector<DrawingWire>& wires)
  {
    std::vector<std::pair<double, double>> points;
    for (const DrawingWire& wire : wires)
    {
      if (std::fabs(wire.curve_deg) > 0.001)
      {
        append_drawing_arc(points, wire.x1_mm, wire.y1_mm, wire.x2_mm, wire.y2_mm, wire.curve_deg);
      }
      else
      {
        points.emplace_back(wire.x1_mm, wire.y1_mm);
      }
    }
    if (!points.empty()) { points.push_back(points.front()); }
    return points;
  }

  double polygon_area_m2_from_mm(const std::vector<std::pair<double, double>>& points)
  {
    double area_mm2 = 0.0;
    for (unsigned int i = 0; i + 1 < points.size(); ++i)
    {
      area_mm2 += points[i].first * points[i + 1].second - points[i + 1].first * points[i].second;
    }
    return 0.5 * std::fabs(area_mm2) * 1.0e-6;
  }

  FrameContour make_frame_contour(unsigned int module, bool outer_boundary)
  {
    const auto xy_points = rasterize_drawing_wires(frame_drawing_wires(module, outer_boundary));
    FrameContour contour;
    contour.points.reserve(xy_points.size());
    contour.r_min_cm = std::numeric_limits<double>::max();
    contour.r_max_cm = -std::numeric_limits<double>::max();
    contour.phi_min = std::numeric_limits<double>::max();
    contour.phi_max = -std::numeric_limits<double>::max();
    contour.area_m2 = polygon_area_m2_from_mm(xy_points);

    std::vector<double> phi_values;
    phi_values.reserve(xy_points.size());
    double prev_phi = 0.0;
    for (unsigned int i = 0; i < xy_points.size(); ++i)
    {
      const double dx = xy_points[i].first - frameOriginXMm[module];
      const double dy = xy_points[i].second - frameOriginYMm[module];
      const double r_cm = std::sqrt(dx * dx + dy * dy) / 10.0;
      double phi = std::atan2(dy, dx);
      if (i > 0)
      {
        while (phi - prev_phi > std::numbers::pi) { phi -= 2.0 * std::numbers::pi; }
        while (phi - prev_phi < -std::numbers::pi) { phi += 2.0 * std::numbers::pi; }
      }
      prev_phi = phi;
      contour.r_min_cm = std::min(contour.r_min_cm, r_cm);
      contour.r_max_cm = std::max(contour.r_max_cm, r_cm);
      phi_values.push_back(phi);
    }

    const auto [phi_min_iter, phi_max_iter] = std::minmax_element(phi_values.begin(), phi_values.end());
    const double phi_center = 0.5 * (*phi_min_iter + *phi_max_iter);
    for (unsigned int i = 0; i < xy_points.size(); ++i)
    {
      const double dx = xy_points[i].first - frameOriginXMm[module];
      const double dy = xy_points[i].second - frameOriginYMm[module];
      const double r_cm = std::sqrt(dx * dx + dy * dy) / 10.0;
      const double phi_rel = phi_values[i] - phi_center;
      contour.phi_min = std::min(contour.phi_min, phi_rel);
      contour.phi_max = std::max(contour.phi_max, phi_rel);
      contour.points.push_back({r_cm, phi_rel});
    }
    return contour;
  }

  const std::array<ModuleFrameGeometry, nTpcModules>& frame_geometries()
  {
    static const std::array<ModuleFrameGeometry, nTpcModules> geometries = []
    {
      std::array<ModuleFrameGeometry, nTpcModules> out{};
      for (unsigned int module = 0; module < nTpcModules; ++module)
      {
        out[module].outer_boundary = make_frame_contour(module, true);
        out[module].inner_boundary = make_frame_contour(module, false);
        out[module].area_m2 = std::max(0.0, out[module].outer_boundary.area_m2 - out[module].inner_boundary.area_m2);
      }
      return out;
    }();
    return geometries;
  ...
  std::vector<double> frame_observation_r_edges(double inner_cm, double outer_cm)
  {
    ...
    const auto& geometries = frame_geometries();
    std::vector<double> breakpoints{inner_cm, outer_cm};
    for (const ModuleFrameGeometry& geometry : geometries)
    {
      for (const FrameContour* contour : {&geometry.outer_boundary, &geometry.inner_boundary})
      {
        breakpoints.push_back(std::clamp(contour->r_min_cm - fine_padding_cm, inner_cm, outer_cm));
        breakpoints.push_back(std::clamp(contour->r_min_cm, inner_cm, outer_cm));
        breakpoints.push_back(std::clamp(contour->r_min_cm + fine_padding_cm, inner_cm, outer_cm));
        breakpoints.push_back(std::clamp(contour->r_max_cm - fine_padding_cm, inner_cm, outer_cm));
        breakpoints.push_back(std::clamp(contour->r_max_cm, inner_cm, outer_cm));
        breakpoints.push_back(std::clamp(contour->r_max_cm + fine_padding_cm, inner_cm, outer_cm));
      }
    }
    ...
    for (unsigned int i = 0; i + 1 < breakpoints.size(); ++i)
    {
      ...
      for (const ModuleFrameGeometry& geometry : geometries)
      {
        fine = fine || (mid_cm >= geometry.outer_boundary.r_min_cm - fine_padding_cm &&
                        mid_cm <= geometry.outer_boundary.r_max_cm + fine_padding_cm);
      }
  ...
  double frame_sector_center(unsigned int side, unsigned int sector, double frame_reference_phi)
  {
    ...
    const double reference_center = side == 1U ? frame_reference_phi : -frame_reference_phi;
    double phi = reference_center + (static_cast<int>(sector) - static_cast<int>(frameReferenceSector)) * sector_width;
  ...
  std::vector<double> frame_phi_edges(unsigned int, unsigned int side, double frame_reference_phi, double lo)
  {
    ...
    for (unsigned int sector = 0; sector < nTpcSectors; ++sector)
    {
      ...
      for (const ModuleFrameGeometry& geometry : geometries)
      {
        const double phi_low = side == 1U ? center + geometry.outer_boundary.phi_min : center - geometry.outer_boundary.phi_max;
        const double phi_high = side == 1U ? center + geometry.outer_boundary.phi_max : center - geometry.outer_boundary.phi_min;
        add_edge(phi_low);
        add_edge(phi_high);
      }
    }
    ...
    for (unsigned int i = 0; i + 1 < breakpoints.size(); ++i)
    {
      ...
      for (unsigned int sector = 0; sector < nTpcSectors; ++sector)
      {
        ...
        for (const ModuleFrameGeometry& geometry : geometries)
        {
          const double phi_low = side == 1U ? center + geometry.outer_boundary.phi_min : center - geometry.outer_boundary.phi_max;
          const double phi_high = side == 1U ? center + geometry.outer_boundary.phi_max : center - geometry.outer_boundary.phi_min;
          for (const double boundary : {phi_low, phi_high})
          {
...
PHGarfieldRossegger::SourceGrid PHGarfieldRossegger::buildSourceGrid() const
{
  if (m_useFrameChargeModel)
  {
    ...
    for (const ModuleFrameGeometry& geometry : geometries)
    {
      add_interval(geometry.outer_boundary.r_min_cm, geometry.outer_boundary.r_max_cm);
    }
    ...
    for (unsigned int ir = 0; ir < source_grid.r_centers_m.size(); ++ir)
    {
      ...
      for (unsigned int module = 0; module < geometries.size(); ++module)
      {
        const ModuleFrameGeometry& geometry = geometries[module];
        if (rcm >= geometry.outer_boundary.r_min_cm && rcm <= geometry.outer_boundary.r_max_cm)
        {
...
namespace
{
  bool point_in_contour(const FrameContour& contour, double r_cm, double phi_rel)
  {
    bool inside = false;
    for (unsigned int i = 0, j = contour.points.size() - 1; i < contour.points.size(); j = i++)
    {
      const FramePoint& pi = contour.points[i];
      const FramePoint& pj = contour.points[j];
      const bool crosses = ((pi.phi_rel > phi_rel) != (pj.phi_rel > phi_rel)) &&
                           (r_cm < (pj.r_cm - pi.r_cm) * (phi_rel - pi.phi_rel) / (pj.phi_rel - pi.phi_rel) + pi.r_cm);
      if (crosses) { inside = !inside; }
    }
    return inside;
  }

  bool point_in_frame_geometry(const ModuleFrameGeometry& geometry, double r_cm, double phi_rel)
  {
    if (r_cm < geometry.outer_boundary.r_min_cm || r_cm > geometry.outer_boundary.r_max_cm ||
        phi_rel < geometry.outer_boundary.phi_min || phi_rel > geometry.outer_boundary.phi_max)
    {
      return false;
    }
    return point_in_contour(geometry.outer_boundary, r_cm, phi_rel) &&
           !point_in_contour(geometry.inner_boundary, r_cm, phi_rel);
  }
}  // namespace
                                                                                         ...
                                                                                         unsigned int side) const
{
  ...
  for (const ModuleFrameGeometry& geometry : geometries) { reference_piece_area += geometry.area_m2; }
  reference_piece_area /= static_cast<double>(geometries.size());
      ...
      for (unsigned int irs = 0; irs < n_r_samples; ++irs)
      {
        ...
        for (unsigned int ips = 0; ips < n_phi_samples; ++ips)
        {
          ...
          for (unsigned int sector = 0; sector < nTpcSectors; ++sector)
          {
            ...
            if (!point_in_frame_geometry(geometry, r_cm, phi_rel)) { continue; }
            geometry_fraction += inv_samples;
            if (m_frameChargeWeighting == FrameChargeWeighting::EqualChargePerPiece)
            {
              weight += inv_samples * reference_piece_area / geometry.area_m2;
            }
            else
            {
              weight += inv_samples;
